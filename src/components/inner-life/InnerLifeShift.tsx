...
export default function InnerLifeShift({ data }: { data?: any }) {
  const tb = getPageSection(data, 3);
  const shiftLabel = tb.shiftLabel || "The Shift";
  const shiftTitle1 = tb.shiftTitle1 || "What you're experiencing ";
  const shiftHighlight = tb.shiftHighlight || "is not random.";
  const shiftP1 = tb.shiftP1 || "There are systems within you shaping how you think, respond, and move. When you begin to see them, things start to make more sense.";
