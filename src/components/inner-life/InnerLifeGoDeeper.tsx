...
export default function InnerLifeGoDeeper({ data }: { data?: any }) {
  ...
  const goDeeperTitle = tb.goDeeperTitle || "If this resonates, the next step is not to force change.";
  const goDeeperP1 = tb.goDeeperP1 || "It is to understand yourself more clearly.";
  const goDeeperBtn = tb.goDeeperBtn || "Go deeper into Life";
  const goDeeperBtnLink = tb.goDeeperBtnLink || "/thoughts?category=life";
