"use client";

import { motion } from "framer-motion";
import { useInView } from "framer-motion";
import { useRef } from "react";
import Link from "next/link";
import {
  Server,
  Database,
  Shield,
  Monitor,
  Network,
  ArrowRight,
} from "lucide-react";

export default function Infrastructure() {
  const ref = useRef(null);
  const isInView = useInView(ref, { once: true, amount: 0.3 });

  const infrastructureComponents = [
    {
      icon: Server,
      title: "Cloud Infrastructure",
      description: "Scalable cloud architecture with 99.9% uptime",
      metrics: "Auto-scaling, enterprise-grade reliability",
    },
    {
      icon: Database,
      title: "Data Architecture",
      description: "Modern data stack for real-time analytics",
      metrics: "Petabyte scale, real-time processing",
    },
    {
      icon: Shield,
      title: "Security & Compliance",
      description: "Enterprise security with SOC 2 compliance",
      metrics: "Zero incidents, GDPR compliant",
    },
    {
      icon: Monitor,
      title: "Telemetry & Monitoring",
      description: "Comprehensive system observability",
      metrics: "10M+ events/day, 24/7 monitoring",
    },
    {
      icon: Network,
      title: "Attribution & Routing",
      description: "Multi-touch attribution and intelligent data routing",
      metrics: "91% attribution accuracy, < 200ms latency",
    },
  ];

  return (
    <section
      id="infrastructure"
      className="py-20 bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-800 dark:to-slate-900"
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div
          ref={ref}
          initial={{ opacity: 0, y: 30 }}
          animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
          transition={{ duration: 0.6 }}
          className="text-center mb-16"
        >
          <h2 className="text-4xl md:text-5xl font-black mb-6">
            Infrastructure & Technology Stack
          </h2>
          <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto">
            Enterprise-grade infrastructure that powers scalable growth systems with 
            modern cloud architecture, comprehensive monitoring, and robust security
          </p>
        </motion.div>

        {/* Infrastructure Components Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {infrastructureComponents.map((component, index) => (
            <motion.div
              key={component.title}
              initial={{ opacity: 0, y: 30 }}
              animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 30 }}
              transition={{ duration: 0.6, delay: index * 0.1 }}
              className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-xl"
            >
              <div className="w-12 h-12 bg-primary rounded-2xl flex items-center justify-center mb-6">
                <component.icon className="w-6 h-6 text-white" />
              </div>
              <h3 className="text-xl font-bold mb-4">{component.title}</h3>
              <p className="text-gray-600 dark:text-gray-300 mb-6">{component.description}</p>
              <div className="bg-blue-50 dark:bg-slate-700 p-4 rounded-lg">
                <p className="text-sm font-semibold text-primary">{component.metrics}</p>
              </div>
            </motion.div>
          ))}
        </div>

        {/* Learn More CTA */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={isInView ? { opacity: 1, y: 0 } : { opacity: 0, y: 20 }}
          transition={{ duration: 0.6, delay: 0.6 }}
          className="text-center"
        >
          <Link 
            href="/infrastructure"
            className="inline-flex items-center bg-primary hover:bg-primary/90 text-white px-8 py-4 rounded-xl font-semibold transition-all transform hover:scale-105 shadow-lg hover:shadow-xl"
          >
            Learn More About Infrastructure
            <ArrowRight className="w-5 h-5 ml-2" />
          </Link>
        </motion.div>
      </div>
    </section>
  );
}
